...
const ArticleTagDao     = require('../dao/ArticleTagDao');
const responseHandler   = require('../helper/responseHandler');
const logger            = require('../config/logger');

class ArticleTagService {
  constructor() {
    this.dao = new ArticleTagDao();
  }

  createAssociation = async (body) => {
    try {
      const at = await this.dao.create(body);
      return responseHandler.returnSuccess(httpStatus.CREATED, 'Association created', at);
    } catch (e) {
      logger.error(e);
